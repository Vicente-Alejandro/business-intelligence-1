...
# Database population counts
# Adjust these values as needed to control the amount of data generated for each table.
...
def populate_database():
    ...
    for id_prd in product_ids:
        ...
        products.append((id_prd, fake.catch_phrase()[:70], random.randint(1, TOTAL_PRODUCT_LINES), 
                          f"1:{random.choice()}", random.randint(0, 1000), 
    ...
    for i, doc in enumerate(employee_docs):
        # The first employee is the CEO, others report to a random previous employee
    ...
    connection.commit()
    total_inserted = len(lines) + len(offices) + len(products) + len(employees) + len(customers) + len(payments) + len(orders) + len(details_list)
